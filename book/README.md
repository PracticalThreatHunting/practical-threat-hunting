# Book Resources

This directory contains edition information, errata, and companion references for the **Practical Threat Hunting** series by Grant Halden. The manuscripts are not stored in this public repository.

## Book 1 — Modern Techniques for the AI-Augmented SOC

**Practical Threat Hunting: Modern Techniques for the AI-Augmented SOC**<br>
*25 Hands-On Hunts Across Endpoint, Identity, Cloud, Microsoft 365, and AI Systems*

Status: **Published — First Edition, August 2026**

- [Kindle edition](https://www.amazon.com/dp/B0HFGYN38J) — ASIN `B0HFGYN38J`
- [Paperback edition](https://www.amazon.com/dp/B0HFHD55GB) — ASIN `B0HFHD55GB`
- Paperback ISBN: `9798192696538`
- [Hunt implementations](../hunts/README.md#book-1--modern-techniques-for-the-ai-augmented-soc)
- [Figures](../assets/figures/README.md#book-1--modern-techniques-for-the-ai-augmented-soc)

## Book 2 — Endpoint and Identity Threats

**Practical Threat Hunting: Endpoint and Identity Threats**<br>
*24 Hands-On Hunts for the AI-Augmented SOC*

Status: **Scheduled for publication September 3, 2026 — Kindle pre-order live**

- [Kindle edition](https://www.amazon.com/dp/B0HFLS6MVC) — ASIN `B0HFLS6MVC`
- Paperback — release scheduled September 3, 2026 — ASIN `B0HFNDYLTJ`
- Paperback ISBN: `9798193349570`
- [Edition and companion information](endpoint-and-identity-threats.md)
- [Hunt implementations](../hunts/book-02-endpoint-and-identity/README.md)
- [Figures](../assets/figures/README.md#book-2--endpoint-and-identity-threats)
- [Technical source notes](../resources/book-02-source-notes.md)

## Book 3 — Cloud and SaaS Environments

**Practical Threat Hunting: Cloud and SaaS Environments**<br>
*24 Hands-On Hunts Across AWS, Azure, Microsoft 365, Google Workspace, and GitHub*

Status: **Published — First Edition, August 2026**

- Kindle ASIN: pending repository confirmation
- [Paperback edition](https://www.amazon.com/dp/B0HFZGYX3X) — ASIN `B0HFZGYX3X`
- Paperback ISBN: `9798193986522`
- Amazon A+ content published August 21, 2026 and applied to two Book 3 ASINs
- [Edition and companion information](cloud-and-saas-environments.md)
- [Hunt index and reference queries](../hunts/book-03-cloud-and-saas/README.md)
- [Figures](../assets/figures/README.md#book-3--cloud-and-saas-environments)
- [Amazon A+ content package](../assets/a-plus/book-03-cloud-saas/README.md)
- [Technical source notes](../resources/book-03-source-notes.md)

## Shared Records

- [Edition notes](edition-notes.md)
- [Errata](errata.md)

Repository content is versioned independently from each printed edition. Check the relevant edition notes before operational use.
